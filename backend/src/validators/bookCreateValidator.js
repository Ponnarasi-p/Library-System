...
exports.validateCreateBook = (data) => {
  if (!data.book_title || data.book_title.trim() === "") {
    throw { status: 400, message: ERROR.VALIDATION_FAILED, description: "Book title is required" };
  }

  if (!data.author_name || data.author_name.trim() === "") {
    throw { status: 400, message: ERROR.VALIDATION_FAILED, description: "Author name is required" };
  }
